...
namespace MyFps
{
    ...
    public class PlayerMove : MonoBehaviour
    {
        ...
        [SerializeField]
        private float gravity = -15.0f;     //중력, (물리 기본 값은 -9.81f)
        [SerializeField]
        private float jumpTimeout = 0.1f;   //점프 키입력 처리

        [SerializeField]
        private float verticalVelocity;     //y축 연산 결과 
        ...
        [SerializeField]
        private float groundedOffset = -0.14f;  //체크 포지션 조정 값
        [SerializeField]
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            {
                Gizmos.color = new Color(1f, 0f, 0f, 0.35f);    //붉은색 투명
            }
            //체크 위치 설정
        ...
        private void JumpGravity()
        {
            if(grounded)
            {
                //지면에 있을 때 verticalVelocity (감산되지 않도록) 값 고정
        ...
        {
            //CharacterController의 그라운드 체크 값 가져오기 (도 할 수 있음)
        ...
        private void Move()
        {
            ...
            _controller.Move(inputDirection.normalized * Time.deltaTime * speed + new Vector3(0f, verticalVelocity, 0f) * Time.deltaTime);
