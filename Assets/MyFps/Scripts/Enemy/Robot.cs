...
namespace MyFps
{
    ...
    public class Robot : MonoBehaviour, IDamageable
    {
        ...
        [SerializeField]
        private float attackTimer = 2f;     //공격 타이머
        ...
        private void Update()
        {
            ...
            switch (robotState)
            {
                ...
                case RobotState.R_Idle:
                    ...
                    break;
                //플레이어를 향해서 걷기, 플레이어와의 거리가 1.5가 되면 공격 상태로 전환
                case RobotState.R_Walk:
                    transform.Translate(dir.normalized * Time.deltaTime * moveSpeed, Space.World);

                    //플레이어와의 거리가 2 이내가 되면 공격상태로 전환
                ...
                //2초마다 데미지를 5씩 준다 플레이어와의 거리가 1.5가 넘어가면 다시 걷기 상태로 전화
                case RobotState.R_Attack:
                    ...
                    */
                    ...
                    //플레이어와의 거리가 2 이상이면 되면 걷기 상태로 전환
                    if(distance > attackRange)
                    {
                        SetState(RobotState.R_Walk);
                    }
